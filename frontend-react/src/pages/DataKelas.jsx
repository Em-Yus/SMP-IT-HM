...
import { School, Plus, RefreshCw, Trash2, Edit, Users } from 'lucide-react';
import Swal from 'sweetalert2';
import KelasFormModal from '../components/KelasFormModal';
import AnggotaRombelModal from '../components/AnggotaRombelModal';
...
export default function DataKelas() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      console.error(err);
      Swal.fire({ icon: 'error', title: 'Gagal', text: 'Gagal mengambil data kelas' });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleDelete = async (item) => {
    ...
    if (result.isConfirmed) {
      try {
        let query = supabase.from('data_kelas').delete();
        
        if (item.id) query = query.eq('id', item.id);
        else if (item.id_kelas) query = query.eq('id_kelas', item.id_kelas);
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div>
          <h2 className="text-2xl font-bold text-primary flex items-center gap-2">
            <School className="text-primary" /> Data Kelas (Rombel)
          </h2>
          <p className="text-gray-500 text-sm mt-1">Kelola rombongan belajar dan wali kelas.</p>
        </div>
              ...
              {isLoading ? (
                ...
                dataKelas.map((item, idx) => (
                  <tr key={idx} className="hover:bg-gray-50 transition">
                    ...
                    <td className="px-6 py-4 text-gray-600">{item.wali_kelas_nama || 'Belum diatur'}</td>
