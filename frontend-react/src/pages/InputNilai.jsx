...
export default function InputNilai() {
  ...
  const fetchSiswaDanNilai = async () => {
    ...
    try {
      ...
      const combinedData = siswaData.map(siswa => {
        ...
        const tugas = nilaiSiswa.find(n => n.jenis_nilai === 'Tugas');
        ...
        let rawCapaian = (tugas?.id_tujuan_pemb || pts?.id_tujuan_pemb || pas?.id_tujuan_pemb);
        let optimal = [];
        let peningkatan = [];
        if (rawCapaian) {
           try {
        ...
        return {
          ...
          nilai_tugas: tugas ? tugas.nilai_siswa : '',
  ...
  const handleSimpan = async () => {
    ...
    try {
      ...
      dataNilai.forEach(siswa => {
        ...
        if (siswa.nilai_tugas !== '') {
          payload.push({ ...baseRow, jenis_nilai: 'Tugas', nilai_siswa: parseInt(siswa.nilai_tugas) || 0 });
        }
        if (siswa.nilai_pts !== '') {
  ...
  if (dataNilai.length > 0) {
    const scoredData = dataNilai.map(item => {
      const t = parseFloat(item.nilai_tugas) || 0;
      const pts = parseFloat(item.nilai_pts) || 0;
      const pas = parseFloat(item.nilai_pas) || 0;
      return { nipd: item.nipd, total: t + pts + pas };
    });
    scoredData.sort((a, b) => b.total - a.total);
    let currentRank = 1;
    scoredData.forEach((sd, index) => {
      if (index > 0 && sd.total < scoredData[index-1].total) {
        currentRank = index + 1;
      }
      ranksMap[sd.nipd] = currentRank;
    });
  }
            ...
            <table className="w-full text-left border-collapse whitespace-nowrap">
              <thead className="bg-gray-50 text-gray-600 uppercase text-xs font-bold border-b border-gray-100">
                <tr>
                  ...
                  <th className="px-4 py-4 w-20 text-center">Nilai Tugas</th>
                  <th className="px-4 py-4 w-20 text-center">Nilai PTS</th>
                  <th className="px-4 py-4 w-20 text-center">Nilai PAS</th>
                  <th className="px-4 py-4 w-24 text-center text-primary">Peringkat</th>
                  ...
                  <tr key={item.nipd} className="hover:bg-gray-50 transition border-b border-gray-100">
                    ...
                    <td className="px-4 py-4 align-top">
                      <input 
                        ...
                        onChange={(e) => handleInputChange(idx, 'nilai_tugas', e.target.value)}
                        className="w-full px-2 py-2 border rounded-lg text-center focus:ring-2 focus:ring-primary outline-none" 
                      />
                    </td>
                    <td className="px-4 py-4 align-top">
                      <input 
                    ...
                    <td className="px-4 py-4 align-top text-center">
                      <div className="font-bold text-xl text-primary mt-1">{ranksMap[item.nipd] || '-'}</div>
                    </td>
