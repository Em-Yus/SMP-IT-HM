  ...
  useEffect(() => {
    const fetchSiswaAndLedger = async () => {
      ...
      try {
        ...
        // 2. Baca tahun_ajaran dari data_siswa
        const tahunPelajaran = siswaObj.tahun_ajaran;
        if (!tahunPelajaran) {
          setIsLoading(false);
          return;
        }

        // 3. Baca kelas siswa -> tentukan tingkat
        let tingkatSiswa = 7;
        if (siswaObj.kelas) {
        ...
        // 4. Tentukan Tipe Siswa berdasarkan status_siswa di data_siswa
        ...
        // 6. Tampilkan rincian biaya sesuai tingkat siswa
        let extractedItems = [];
        ...
        // 7. Fetch Riwayat Pemasukan (pakai tahunPelajaran yang sama)
        ...
        // 8. Fetch Saldo & Subsidi (pakai tahunPelajaran yang sama)
      ...
      <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 relative overflow-hidden">
        ...
        <div className="flex w-full md:w-auto items-center gap-3 relative z-10 mt-4 md:mt-0">
          <select 
            ...
            className="w-full md:w-48 px-2 sm:px-4 py-2 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none text-sm font-semibold text-gray-700 text-ellipsis overflow-hidden"
          >
