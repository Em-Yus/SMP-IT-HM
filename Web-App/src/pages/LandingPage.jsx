...
export default function LandingPage() {
  ...
  // Fetch Dinamis Biaya PPM dari Supabase
  const [biayaPPM, setBiayaPPM] = useState({ k7: 1000000, k8: 500000, k9: 1000000 });
  const [tahunPPM, setTahunPPM] = useState('2026/2027');
    ...
    const fetchBPM = async () => {
      try {
        const { data, error } = await supabase
          ...
          .select('tipe_siswa, data_anggaran, tahun_pelajaran')
          .eq('semester', 'Tahunan')
          .order('tahun_pelajaran', { ascending: false });

        if (!error && data && data.length > 0) {
          const latestYear = data[0].tahun_pelajaran;
          setTahunPPM(latestYear);

          let k7 = 1000000, k8 = 500000, k9 = 1000000;
          const latestData = data.filter(d => d.tahun_pelajaran === latestYear);

          latestData.forEach(item => {
            const total = (item.data_anggaran || []).reduce((sum, curr) => sum + Number(curr.biaya || 0), 0);
            if (item.tipe_siswa === 'Siswa Baru') k7 = total;
            else if (item.tipe_siswa === 'Pindahan Kelas 8') k8 = total;
            else if (item.tipe_siswa === 'Pindahan Kelas 9') k9 = total;
          });

          setBiayaPPM({ k7, k8, k9 });
        }
      } catch (err) {
        console.error('Failed to fetch Biaya PPM', err);
      }
    };
    fetchBPM();
  }, []);
        ...
        <div className="container mx-auto px-6">
          ...
          <div className="grid md:grid-cols-2 gap-12 items-center mb-16">
            ...
            <div className="reveal-right">
              <h3 className="text-2xl font-bold text-[#2a2c87] mb-4">Tentang SMP IT HM</h3>
              <p className="text-gray-700 leading-relaxed mb-6">
                SMP IT Hidayatul Mubtadi-ien (SMP IT HM) adalah lembaga pendidikan menengah pertama berbasis Islam terpadu yang memadukan kurikulum nasional (Kurikulum Merdeka) dengan penguatan nilai-nilai kepesantrenan. Kami berkomitmen mencetak generasi Qur'ani yang berakhlak mulia, unggul dalam ilmu pengetahuan, dan cakap dalam teknologi digital.
              </p>
          ...
          <div className="grid md:grid-cols-2 gap-16 items-start">
            <div className="reveal">
              ...
              <div className="bg-blue-50 p-6 rounded-lg border-l-4 border-[#85c226] mb-6">
                <h4 className="font-bold text-[#2a2c87] mb-2">Jadwal Pendaftaran ({tahunPPM})</h4>
                <p className="text-gray-700">Januari - Juni {tahunPPM.split('/')[0] || '2027'}</p>
              </div>
            ...
            <div className="bg-white rounded-2xl shadow-2xl p-8 border border-gray-100 reveal relative">
              ...
              <h3 className="text-2xl font-bold text-[#2a2c87] mb-4 text-center">Biaya PPM ({tahunPPM})</h3>
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center border-b pb-2">
                  ...
                  <span className="font-bold text-[#2a2c87] text-xl">Rp {biayaPPM.k7.toLocaleString('id-ID')}</span>
                </div>
                <div className="flex justify-between items-center border-b pb-2">
                  <span className="font-semibold text-gray-700">Kelas 8</span>
                  <span className="font-bold text-[#2a2c87] text-xl">Rp {biayaPPM.k8.toLocaleString('id-ID')}</span>
                </div>
                <div className="flex justify-between items-center pb-2">
                  <span className="font-semibold text-gray-700">Kelas 9</span>
                  <span className="font-bold text-[#2a2c87] text-xl">Rp {biayaPPM.k9.toLocaleString('id-ID')}</span>
                </div>
              </div>
          ...
          <div className="grid md:grid-cols-3 gap-12 mb-12">
            ...
            <div>
              ...
              <ul className="space-y-3 opacity-80">
                ...
                <li className="flex items-center"><i className="fas fa-envelope w-6 text-[#85c226]"></i> smpit.mubtadihidayatul58@gmail.com</li>
              </ul>
            </div>
            <div>
              <h3 className="text-xl font-bold mb-4 text-[#85c226]">Aplikasi Mobile</h3>
              <p className="leading-relaxed opacity-80 mb-4 text-sm">
                Akses portal SIAKAD siswa dan guru lebih cepat dan ringan langsung dari HP Anda.
              </p>
              <a
                href="/SMP-IT-HM.apk"
                download
                className="inline-flex items-center gap-2 bg-[#85c226] hover:bg-green-600 text-white font-bold py-2 px-4 rounded-full transition transform hover:scale-105"
              >
