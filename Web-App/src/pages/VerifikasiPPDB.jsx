...
export default function VerifikasiPPDB() {
  ...
  const handleTerimaSiswa = async (item) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        if (!checkPendaftar) {
          Swal.fire('Gagal', 'Pendaftar ini mungkin sudah diverifikasi oleh operator lain.', 'error');
          fetchData();
          return;
        }
        ...
        console.error(err);
        let errorMessage = err.message || 'Terjadi kesalahan saat menerima siswa.';
        if (errorMessage.toLowerCase().includes('duplicate key value') || errorMessage.toLowerCase().includes('unique constraint')) {
          errorMessage = 'NIPD (Nomor Induk Peserta Didik) atau NISN ini sudah terdaftar di sistem. Data tidak boleh ganda (duplikat).';
        }
  ...
  let filteredData = dataPendaftar.filter(item => 
      ...
      <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 mb-6 flex flex-col md:flex-row gap-4 justify-between items-center">
        ...
        <div className="w-full md:w-auto">
          <select 
            ...
            className="w-full md:w-auto px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-lg focus:ring-2 focus:ring-accent focus:bg-white outline-none text-sm font-medium transition text-gray-700 cursor-pointer"
          >
