...
export default function TagihanSiswa() {
  ...
  const processScanText = async (scannedText) => {
    ...
    try {
      ...
      // Pakai tahun_ajaran dari data_siswa
      const tahun = siswaData.tahun_ajaran || defaultTahun;
      setTahunPelajaran(tahun);
  ...
  const loadLedger = async (siswa = selectedSiswa) => {
    setIsLoading(true);
    try {
      ...
      if (siswa.status_siswa) {
        ...
        if (statusLower === 'baru') {
          ...
          // Contoh: masuk 2025/2026 (entryYear=2025), sekarang 2026/2027 (defaultYear=2026)
          // selisih = 1 tahun, kelas sekarang 9 → masuk di kelas 8 → "Pindahan Kelas 8"
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4 print:hidden">
        ...
        <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
          <div className="flex items-center bg-white border border-gray-200 px-3 py-1.5 rounded-lg shadow-sm">
            <span className="text-sm font-medium text-gray-700">Tahun Ajaran: <span className="font-bold text-indigo-700">{tahunPelajaran}</span></span>
          </div>
          <button
            onClick={handlePrint}
