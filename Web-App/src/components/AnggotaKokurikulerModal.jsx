...
export default function AnggotaKokurikulerModal({ isOpen, onClose, kokurikulerData }) {
  ...
  useEffect(() => {
    if (!isOpen || !kokurikulerData) return;
    ...
    try {
      ...
      if (toAdd.length > 0) {
        const payloadAdd = toAdd.map(id => ({
          kokurikuler_id: kokurikulerData.id,
          siswa_id: id,
      ...
      if (toUpdate.length > 0) {
        ...
        promises.push(
          supabase
            .from('anggota_kokurikuler')
            .upsert(payloadUpdate, { onConflict: 'kokurikuler_id,siswa_id' })
        );
      }
      ...
      Swal.fire({
        icon: 'success',
        title: 'Tersimpan',
        text: 'Data anggota dan capaian projek berhasil diperbarui.',
        timer: 1500,
      ...
      <div className="bg-white rounded-2xl w-full max-w-5xl shadow-2xl flex flex-col h-[90vh] overflow-hidden">
        ...
        <div className="px-6 py-5 border-b border-gray-100 flex justify-between items-center bg-gray-50 shrink-0">
          <div>
            <h2 className="text-xl font-bold text-[#2a2c87] flex items-center gap-2">
              ...
              Kelola Penilaian Projek P5
            </h2>
            <p className="text-gray-500 text-sm mt-1 font-medium">
              Projek: <span className="font-bold text-[#2a2c87]">{kokurikulerData?.nama_projek}</span> 
              &nbsp;|&nbsp; Tema: <span className="text-[#85c226]">{kokurikulerData?.tema}</span>
            </p>
          </div>
          ...
          <table className="w-full text-left border-collapse text-sm">
            <thead className="bg-gray-100 text-gray-600 uppercase text-xs font-bold sticky top-0 z-10 shadow-sm">
              <tr>
                ...
                <th className="px-4 py-3 w-40">Predikat P5</th>
                <th className="px-4 py-3">Catatan Proses / Deskripsi</th>
              </tr>
            </thead>
                      ...
                      <td className="px-4 py-3">
                        {isChecked && (
                          ...
                          >
                            ...
                            <option value="Sangat Berkembang (SB)">Sangat Berkembang (SB)</option>
                            <option value="Berkembang Sesuai Harapan (BSH)">Berkembang Sesuai Harapan (BSH)</option>
                            <option value="Mulai Berkembang (MB)">Mulai Berkembang (MB)</option>
                            <option value="Belum Berkembang (BB)">Belum Berkembang (BB)</option>
                          </select>
                        )}
                      </td>
                      <td className="px-4 py-3">
                        {isChecked && (
                          <input 
                            ...
                            placeholder="Catatan keaktifan siswa pada projek ini..."
                          />
                        )}
                      </td>
                    </tr>
        ...
        <div className="px-6 py-4 border-t border-gray-100 bg-gray-50 flex justify-between items-center shrink-0">
          <div className="text-sm font-medium text-gray-600">
            <span className="font-bold text-[#2a2c87]">{checkedIds.size}</span> siswa dinilai
          </div>
