...
export default function AnggotaRombelModal({ isOpen, onClose, kelasData }) {
  ...
  useEffect(() => {
    ...
    const fetchSiswa = async () => {
      ...
      try {
        ...
        data?.forEach(siswa => {
          if (siswa.kelas === kelasData.nama_kelas) {
  ...
  const uniqueClasses = ['Semua', 'Belum ada kelas', ...new Set(dataSiswa.map(item => item.kelas).filter(Boolean))].sort();
  ...
  useEffect(() => {
    ...
    let filtered = dataSiswa.filter(s => {
      const matchSearch = (s.nama && s.nama.toLowerCase().includes(lowerKeyword)) ||
                          ...
                          (s.kelas && s.kelas.toLowerCase().includes(lowerKeyword));
                          
      let matchKelas = true;
      if (filterKelas === 'Belum ada kelas') {
        matchKelas = !s.kelas;
      } else if (filterKelas !== 'Semua') {
        matchKelas = s.kelas === filterKelas;
      }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const currentClassMembers = dataSiswa.filter(s => s.kelas === kelasData.nama_kelas).map(s => s.id);
      ...
      if (toAdd.length > 0) {
        promises.push(
          supabase
            .from('data_siswa')
            .update({ kelas: kelasData.nama_kelas })
            .in('id', toAdd)
        );
      }
      ...
      if (toRemove.length > 0) {
        promises.push(
          supabase
            .from('data_siswa')
            .update({ kelas: null }) // Atau '-' sesuai standar DB, kita pakai null
            .in('id', toRemove)
        );
      }
      ...
      Swal.fire('Berhasil', 'Anggota rombel berhasil diperbarui.', 'success');
      onClose();
    } catch (err) {
      console.error(err);
        ...
        <div className="bg-primary p-5 rounded-t-2xl flex justify-between items-center shrink-0">
          <div>
             <h3 className="text-lg font-bold text-white flex items-center gap-2">
               <Users size={20} /> Anggota Rombel: {kelasData?.nama_kelas}
             </h3>
             <p className="text-blue-200 text-xs mt-1">Centang siswa untuk memasukkannya ke kelas ini.</p>
          </div>
              ...
              <tbody className="divide-y divide-gray-100">
                {filteredSiswa.length === 0 ? (
                  ...
                  filteredSiswa.map(siswa => {
                    ...
                    const isDifferentClass = siswa.kelas && siswa.kelas !== kelasData.nama_kelas;
                    ...
                    return (
                      <tr key={siswa.id} className={`hover:bg-blue-50 transition cursor-pointer ${isChecked ? 'bg-blue-50/50' : ''}`} onClick={() => handleCheckboxChange(siswa.id)}>
                        ...
                        <td className="px-6 py-3">
                          {siswa.kelas === kelasData.nama_kelas ? (
                            <span className="text-green-600 font-bold bg-green-100 px-2 py-1 rounded-md text-xs">{siswa.kelas} (Ini)</span>
                          ) : isDifferentClass ? (
                            <span className="text-orange-600 font-bold bg-orange-100 px-2 py-1 rounded-md text-xs">{siswa.kelas}</span>
                          ) : (
