...
export const HARI_MAP = ['Minggu', 'Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat', 'Sabtu'];
 ...
 */
export const getOperationalDate = (baseDate) => {
  if (!baseDate) {
    const now = new Date();
    if (now.getHours() >= 18) {
  ...
  // Jika berupa string tanggal 'YYYY-MM-DD' (tanpa jam), jangan geser lagi karena sudah spesifik.
 ...
 */
export const getOperationalDayName = (baseDate) => {
 ...
 */
export const getOperationalDayIndex = (baseDate) => {
  const d = getOperationalDate(baseDate);
  return d.getDay();
};
